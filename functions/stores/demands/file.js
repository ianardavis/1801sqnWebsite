module.exports = function (m, fn) {
    ...
    fn.demands.file.create = function ([demand_id, user]) {
        ...
        function create_file(demand, user) {
                        ...
                        fs.copyFile(
                            ...
                            function (err) {
                                if (err) {
                                    ...
                                    resolve(filename, file, account, user, demand);
        ...
        function write_file(filename, file, account, user, demand) {
            function write_cover_sheet(filename, file, account, raised_by_user, demand_id) {
                return new Promise((resolve, reject) => {
                    Promise.all([
            ...
            return new Promise((resolve, reject) => {
                Promise.all([
                    write_cover_sheet(filename, file, account, user, demand.demand_id),
                    write_items(filename, demand.lines, demand.supplier_id)
                ])
                .then(result => resolve({filename: filename}))
                .catch(err => reject(err));
            });
        };
        ...
        return new Promise((resolve, reject) => {
            ...
            .then((demand) => {
                if (demand.filename && demand.filename !== '') {
                    ...
                    .then(demand.update)
                    .then(demand.reload())
                    .then(filename => resolve(demand.filename))
                    .catch(err => reject(err));
