module.exports = (app, m, pm, op, inc, li, send_error) => {
    app.get('/canteen_items',        li, pm.get, pm.check('access_canteen_items'),              (req, res) => res.render('canteen/items/index'));
    app.get('/canteen_items/:id',    li, pm.get, pm.check('access_canteen_items'),              (req, res) => res.render('canteen/items/show'));
    ...
    app.put('/canteen_items/:id',    li,         pm.check('canteen_item_edit',   {send: true}), (req, res) => {
    ...
    app.post('/canteen_items',       li,         pm.check('canteen_item_add',    {send: true}), (req, res) => {
        m.canteen_items.create(req.body.item)
        .then(item => res.send({success: true, message: `Item added: ${item.item_id}`}))
        .catch(err => send_error(res, err));
    });

    app.delete('/canteen_items/:id', li,         pm.check('canteen_item_delete', {send: true}), (req, res) => {
        m.canteen_items.findOne({
            where: {item_id: req.params.id},
            attributes: ['item_id']
        })
