module.exports = (app, inc, pm, m, li, send_error) => {
    ...
    app.put('/notes',        li, pm.check('note_edit'),    (req, res) => {
        ...
        .then(note => {
            if     (!note)        send_error(res, 'Note not found')
            else if (note.system) send_error(res, 'System generated notes can not be edited')
            else {
    ...
    app.delete('/notes/:id', li, pm.check('note_delete'),  (req, res) => {
        ...
        .then(note => {
            if      (!note)       send_error(res, 'Note not found')
            else if (note.system) send_error(res, 'System generated notes can not be deleted')
            else {
                return note.destroy()
