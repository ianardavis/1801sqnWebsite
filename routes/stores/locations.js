module.exports = (app, m, pm, op, inc, li, send_error) => {
    app.get('/locations/:id', li, pm.get('access_locations'),   (req, res) => res.render('stores/locations/show'));
    app.get('/get/location',  li, pm.check('access_locations'), (req, res) => {
        m.locations.findOne({where: req.query})
        .then(location => res.send({success: true, result: location}))
        .catch(err => send_error(res, err));
    });
    app.get('/get/locations', li, pm.check('access_locations'), (req, res) => {
        m.locations.findAll({where: req.query})
        .then(locations => res.send({success: true, result: locations}))
        .catch(err => send_error(res, err));
    });

    app.put('/locations/:id', li, pm.check('location_edit'),    (req, res) => {
