...
module.exports = (app, al, inc, pm, m) => {
    ...
    app.get('/stores/get/request',         pm, al('access_requests', {allow: true, send: true}), (req, res) => {
        ...
        .then(request => {
            if (!request)                                    res.send({success: false, message: 'Request not found'})
                ...
                !req.allowed &&
                request.user_id_request !== req.user.user_id
            )                                                res.send({success: false, message: 'Permission denied'})
            else                                             res.send({success: true,  result: request});
        })
        .catch(err => res.error.send(err, res));
    });
    app.get('/stores/get/request_actions', pm, al('access_requests',              {send: true}), (req, res) => {
    ...
    app.delete('/stores/requests/:id',     pm, al('request_delete',  {allow: true, send: true}), (req, res) => {
        ...
        .then(request => {
            ...
            else if (request._status !== 1 && request._status !== 2)               res.send({success: false, message: 'Only requested and approved lines can be cancelled'})
            else {
                let actions = [];
                actions.push(request.update({_status: 0}));
                actions.push(
                    m.stores.request_actions.create({
                        request_id: request.request_id,
                        _action: 'Cancelled',
                        user_id: req.user.user_id
                    })
