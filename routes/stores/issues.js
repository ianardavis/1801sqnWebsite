module.exports = (app, fn) => {
    ...
    app.put('/issues',          fn.loggedIn(), fn.permissions.check('issuer'),              (req, res) => {
        fn.issues.update(req.body.lines, req.session.site_id, req.user.user_id)
        .then(message => res.send({success: true, message: message}))
        .catch(err => fn.sendError(res, err));
    });

    app.put('/issues/:id/qty',  fn.loggedIn(), fn.permissions.check('issuer'),              (req, res) => {
        fn.issues.changeQty(req.params.id, req.session.site_id, req.body.qty , req.user.user_id)
    ...
    app.put('/issues/:id/size', fn.loggedIn(), fn.permissions.check('issuer'),              (req, res) => {
        fn.issues.changeSize(req.params.id, req.session.site_id, req.body.size_id, req.user.user_id)
    ...
    app.put('/issues/:id/mark', fn.loggedIn(), fn.permissions.check('issuer'),              (req, res) => {
        fn.issues.markAs(req.params.id, req.session.site_id, req.body.issue.status, req.user.user_id)
        .then(message => res.send({success: true, message: message}))
        .catch(err => fn.sendError(res, err));
    });

    app.delete('/issues/:id',   fn.loggedIn(), fn.permissions.check('access_stores'),       (req, res) => {
        fn.issues.cancelOwn(req.params.id, req.session.site_id, req.user.user_id)
