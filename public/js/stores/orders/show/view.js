    ...
    get(
        {
            ...
            onFail: function () {window.location.href = '/orders'}
        },
        function (order, options) {
            set_innerText({id: 'size_desc', text: order.size._size});
            set_attribute({id: 'size_desc_link',  attribute: 'href', value: `/sizes/${order.size_id}`});
            set_innerText({id: 'item_name', text: order.size.item._description});
            set_attribute({id: 'item_name_link',  attribute: 'href', value: `/items/${order.size.item_id}`});
