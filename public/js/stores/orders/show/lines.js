...
showLines = (lines, options) => {
    ...
    lines.forEach(line => {
        try {
            ...
            add_cell(row, {text: statuses[line._status] || 'Unknown', id: `cell_status_${line.line_id}`});
...
function add_modal (line) {
    ...
    get_line_notes({id: line.line_id, table: 'order_lines'});
    ...
    created.innerText = 'Line created';
    nav_body_2.appendChild(document.createElement('hr')); 
    nav_body_2.appendChild(created); 
    nav_body_2.appendChild(new Input_Group({title: 'Date', text: print_date(line.createdAt, true)}).e);
    nav_body_2.appendChild(new Input_Group({title: 'User', text: print_user(line.user), link: `/stores/users/${line.user_id}`}).e);
    line.actions.forEach(action => {
        let header = document.createElement('h5');
        header.innerText = action._action;
        nav_body_2.appendChild(document.createElement('hr')); 
        nav_body_2.appendChild(header); 
        nav_body_2.appendChild(new Input_Group({title: 'Date',    text: print_date(action.createdAt)}).e);
        nav_body_2.appendChild(new Input_Group({title: 'Line ID', text: action.action_line_id,   link: `/stores/${action._action.toLowerCase()}_lines/${action.action_line_id}`}).e);
        nav_body_2.appendChild(new Input_Group({title: 'User',    text: print_user(action.user), link: `/stores/users/${action.user_id}`}).e);
    });
};