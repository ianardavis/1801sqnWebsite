function getStock() {
    ...
    .then(function ([stock, options]) {
        set_breadcrumb({text: stock.size.item.description,                                      id: 'breadcrumb_item', href: `/items/${stock.size.item_id}`});
        set_breadcrumb({text: `${print_size_text(stock.size.item)}: ${print_size(stock.size)}`, id: 'breadcrumb_size', href: `/sizes/${stock.size_id}`});
        set_breadcrumb({text: `Location: ${stock.location.location}`})
