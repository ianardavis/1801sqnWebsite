...
function viewAction(action_id) {
    ...
    .then(function ([action, options]) {
        ...
        set_innerText({id: 'action_issue',         value: (action.issue         ? print_user(action.issue.user_issue) : '')});
        set_innerText({id: 'action_loancard',      value: (action.loancard      ? action.loancard_id                  : '')});
        ...
        set_href({id: 'action_loancard_link',      value: (action.loancard      ? `/loancards/${action.loancard_id}`               : '')});
        set_href({id: 'action_loancard_line_link', value: (action.loancard_line ? `/loancards/${action.loancard_line.loancard_id}` : '')});
        set_href({id: 'action_order_link',         value: (action.demand        ? `/orders/${action.order_id}`                     : '')});
    });
};
