module.exports = function (m, issues) {
    issues.create = function (options = {}) {
        return new Promise((resolve, reject) => {
            ...
            .then(user => {
                if      (!user)                                        resolve({success: false, message: 'User not found'})
                    ...
                    .then(([issue, created]) => {
                        if (created) resolve({success: true, message: 'Issue created',              issue: {issue_id: issue.issue_id, created: created}})
                        else         resolve({success: true, message: 'Draft issue already exists', issue: {issue_id: issue.issue_id, created: created}});
                    })
                            ...
                            .then(lines => {
                                if      (size._serials && (!options.serial_id || options.serial_id === '')) resolve({success: false, message: 'You must specify a serial #'})
                                    ...
                                    if (size._serials) { //If serials required
                                        verify_search = m.serials.findOne({
                                            where: {
                                    ...
                                    if (verify_search) {
                                        ...
                                        .then(result => {
                                            if (!result) resolve({success: false, message: 'Stock/Serial not found'});
                                                ...
                                                .then(issue_line => resolve({success: true, message: 'Issue line created', line: {line_id: issue_line.line_id}}))
                                                .catch(err => reject(err));
                                            };
                                        })
                                        .catch(err => reject(err));
