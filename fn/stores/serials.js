                ...
                if (!size) {
                    ...
                    .then(([serial, created]) => {
                        if (created) {
                            ...
                            (
                                !serial.location_id ||
                                String(serial.location_id).trim() === ''
                            ) && 
                            (
                                !serial.issue_line_id ||
                                String(serial.issue_line_id).trim() === ''
                            )
                        ) {
                            resolve({
                                success: true,
                                message: 'Serial exists, not issued or in stock',
                                serial_id: serial.serial_id
                            });
                        } else {
                            resolve({
                                success: false,
                                message: 'Serial exists, issued or in stock',
                                serial_id: serial.serial_id
                            });
                        };
                    })
